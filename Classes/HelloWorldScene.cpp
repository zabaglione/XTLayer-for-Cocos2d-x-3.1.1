...
bool HelloWorld::init()
{
    ...
    // XTLayerによるジェスチャー入力を有効にする
    xtTouchEnabled(true);
    // シングルタッチ
    xtTouchMode(Touch::DispatchMode::ONE_BY_ONE);
    // マルチタッチ
//    xtTouchMode(Touch::DispatchMode::ALL_AT_ONCE);
...
void HelloWorld::xtTapGesture(cocos2d::Point position)
{
    ...
    CCLOG("point(%f, %f)", position.x, position.y);
}
...
{
    static std::vector<std::string> dirstr = {"LEFT","RIGHT","UP","DOWN"};
    CCLOG("%s", __FUNCTION__);
    CCLOG("direction=%s distance=%f speed=%f", dirstr[direction].c_str(), distance, speed);
}
